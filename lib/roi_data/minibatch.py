...
def get_minibatch(roidb):
    ...
    if cfg.SEM.SEM_ON or cfg.DISP.DISP_ON:
        im_blob, im_scales = _get_multitask_image_blob(roidb, prefix='image_L')
        blobs['data'] = im_blob
    if cfg.SEM.SEM_ON:
        valid  = roi_data.semseg.add_sem_blobs(blobs, im_scales, roidb, cv2.INTER_NEAREST)
    if cfg.DISP.DISP_ON:
    ...
    return blobs, valid
def _minibatch_right_image_blob(roidb, target_sizes, prefix,  followed=True):
    """Builds an input blob from the images in the roidb at the targe
    scales.
    """
    ...
    for i in range(num_images):
        ...
        im, _ = blob_utils.prep_multitask_im_for_blob(
            im, cfg.PIXEL_MEANS, [target_sizes[i]], cfg.TRAIN.MAX_SIZE, followed)
        processed_ims.append(im[0])
    blob = blob_utils.im_list_to_blob(processed_ims)
    return blob
def _get_multitask_image_blob(roidb, prefix, followed=False):
    ...
    for i in range(num_images):
        ...
        im, im_scale = blob_utils.prep_multitask_im_for_blob(
            im, cfg.PIXEL_MEANS, [target_size], cfg.TRAIN.MAX_SIZE, followed)
