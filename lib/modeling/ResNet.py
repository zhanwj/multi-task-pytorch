...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts):
        ...
        self.conv_r = nn.Conv2d(337, 512, kernel_size=3, stride=1,padding=1, bias=False)
    ...
    def _correlate(self,input1, input2):
    ...
    def forward(self, x):
        if cfg.DISP.DISP_ON:
            ...
            #corr=self._correlate(left,right)
            conv_rdi=self.conv_rdi(left)
            x=torch.cat((conv_rdi,corr),dim=1)
            x=self.relu(self.bn4(self.conv_r(x)))
