...
import modeling.dispSeg_heads as dispSeg_heads
...
class Generalized_3DSD(SegmentationModuleBase):
    ...
    def disp_loss(self, pred, label_info, label_i=0, sparse=True, shape=(30,30)):
        targe = label_info['disp_label_0']
        if sparse:
            targe = F.adaptive_max_pool2d(targe, shape)
        #calucate disp output
        disp_scans = label_info['disp_scans'].repeat(pred.shape[0]//cfg.TRAIN.IMS_PER_BATCH, 1,1,1)
