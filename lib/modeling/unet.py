...
class SubUberNet(nn.Module):
    def __init__(self, blocks, index):
        ...
        self.encoder_part = []
        for ilayer in range(index, 5):
            self.encoder_part.append([])
            self.encoder_part[ilayer]=conv3x3_bn_relu(layer_dims[5-num_layer],
                    layer_dims[5-num_layer])
            self.encoder_part[ilayer]=nn.ModelList(self.encoder_part[ilayer])
        self.encoder_part = nn.ModelList(self.encoder_part)

        # fc6
        self.out1 = nn.Conv2d(512, 512, 1, padding=0)
