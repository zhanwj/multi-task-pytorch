...
class Affinity_Propagate(nn.Module):
    ...
    def eight_way_propagation(self, weight_matrix, blur_matrix, kernel):
        ...
        self.avg_conv = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=kernel, groups=self.groups, stride=1, padding=(kernel-1)/2, bias=False)
        ...
        self.sum_conv = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=kernel,groups=self.groups, stride=1, padding=(kernel-1)/2, bias=False)
