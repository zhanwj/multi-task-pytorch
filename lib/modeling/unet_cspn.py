...
import modeling.cspn as post_process
from torch.autograd import Variable
import modeling.update_model
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, up_proj_block):
        ...
        self.conv1_1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        ...
        wid, hei = cfg.SEM.INPUT_SIZE
        assert wid%32==0 and hei%32==0, 'input size must be 32x'
        self.gud_up_proj_layer1 = self._make_gud_up_conv_layer(Gudi_UpProj_Block, 2048, 1024, wid//16 , hei//16 )
        self.gud_up_proj_layer2 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 1024, 512, wid//8, hei//8)
        self.gud_up_proj_layer3 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 512, 256, wid//4, hei//4)
        self.gud_up_proj_layer4 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 256, 64,  wid//2, hei//2)
        self.gud_up_proj_layer5 = self._make_gud_up_conv_layer(Simple_Gudi_UpConv_Block_Last_Layer, 64, cfg.MODEL.NUM_CLASSES, wid, hei)
        self.gud_up_proj_layer6 = self._make_gud_up_conv_layer(Simple_Gudi_UpConv_Block_Last_Layer, 64, 8, wid, hei)
    ...
    def forward(self, x, sparse_depth=None):
        ...
        pred_coarse= self.gud_up_proj_layer5(x)
        
        guidance = self.gud_up_proj_layer6(x)
        x = self.post_process_layer(guidance, pred_coarse, sparse_depth)
