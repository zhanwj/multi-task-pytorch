...
import modeling.dispSeg_heads_original as semseg_heads
...
class Generalized_SEGDISP(SegmentationModuleBase):
    ...
    def forward(self, data, **label_info):
        ...
        if self.training: # training
            ...
            """
            if loss_disp <= 3. and self.flag <= 10:
                res = torch.cat((pred_disp[0,0], label_info['%s_0'%cfg.DISP.OUTPUT_PREFIX][0,::step, ::step]),dim=1).cpu().detach().numpy()
                cv2.imwrite('pred_disp_%d.png'%self.flag, res)
                res =torch.argmax(pred_semseg[0],dim=0).cpu().detach().numpy()
            ...
            pred_semseg = nn.functional.log_softmax(self.conv_last_semseg(pred_semseg), dim =1) #torch.Size([1, 19, 45, 45])
            pred_disp = self.conv_last_disp(pred_disp)


            # semseg deepsup and disp deepsup torch.Size([1, 1024, 90, 90])
            _, pred_disp_deepsup = torch.split(pred_deepsup, 1, dim=0)
            pred_disp_deepsup = self.disp_deepsup(pred_disp_deepsup)

            return_dict['pred_semseg'] = pred_semseg
            return_dict['pred_disp'] = pred_disp
            return_dict['pred_disp_deepsup'] = pred_disp_deepsup
