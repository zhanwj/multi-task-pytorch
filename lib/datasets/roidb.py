...
from .json_dataset_cityscapes import JsonDataset as CJsonDataset
logger = logging.getLogger(__name__)


def combined_roidb_for_training_semseg(dataset_names):
    """Load and concatenate roidbs for one or more datasets.
    The roidb entries are then prepared for use in training,
    which involves caching certain types of metadata for each roidb entry.
    """
    def get_roidb(dataset_name):
        ds = CJsonDataset(dataset_name)
        roidb = ds.get_roidb(
            trainset=cfg.SEM.TRAINSET,
        )
        if cfg.TRAIN.USE_FLIPPED and not cfg.DISP.DISP_ON:
    ...
    roidbs = [get_roidb(args) for args in dataset_names]
    roidb = roidbs[0]
    for r in roidbs[1:]:
        roidb.extend(r)
    ...
    logger.info('semseg or disparity annotations are done')
...
def extend_with_flipped_entries(roidb, dataset):
    ...
    if cfg.SEM.SEM_ON or cfg.DISP.DISP_ON:
        for entry in roidb:
            ...
            tmp_entry['flipped'] = True
            flipped_roidb.append(tmp_entry)
